...
def initialize_review_scraper():
    ...
    for game_id in game_ids_list:
        ...
        url = f"https://store.steampowered.com/appreviews/{game_id}?json=1&language=english&filter=recent&num_per_page=100&purchase_type=all"
        prev_cursor = "*"
        counter = 1
                ...
                if response.status_code == 200:
                    ...
                    if counter == 1:
                        if data["query_summary"]["num_reviews"] == 0:
                            reached_end = True
                            print(f"Game {game_id} has no reviews.")
                            break
                    ...
                    for review in data["reviews"]:
                        if month_before_now > review["timestamp_updated"]:
                            print(f"30-day mark reached, there were {number_of_reviews} reviews for Game ID {game_id}")
                            reached_end = True
                            break
